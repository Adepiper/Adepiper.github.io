...
question1.addEventListener('click', displayColors )
question2.addEventListener('click', joinArrays)
question3.addEventListener('click', findEmployees)
     ...
     Colors.forEach((color, i) => {
        ...
        switch (i) {
            case 1: order = Order[1]
                ...
                break;
                case 2: order = Order[2]
                
                break;
                case 3: order = Order[3]
        ...
        console.log(`${i + 1 + order} choice is ${color}`)
    })
}

 function joinArrays (){
       const myColor = ["Red", "Green", "White", "Black"]
       const a =  myColor.join(',')
       const b= myColor.join('+')
       console.log(a)
       console.log(b)
    }
   ...
   function findEmployees () {
      let job =  employees.filter((employee) => {
            return employee.Job === 'Web Developer'
        })
        let jobAndPlace = employees.filter((employee) => {
            return (employee.Job === 'Web Developer' && employee.Country === 'Nigeria')
        })

        let age = employees.filter((employee) => {
            return employee.age < 20
        })

        let active = employees.filter((employee) => {
            return employee.active = true
        })

        let highestPaid = employees.filter((employee) => {
            return employee.Job === 'Web Developer'
        }).map((employee) => {
